...
def preprocess_function(examples):
    return {
        ...
        "full_format": (f"{examples['question']}\n" f"{examples['chain_of_thought']}"),
    }
...
def load_data(test_size=100, input_type="full_format"):
    ...
    train_dataset = train_dataset.map(preprocess_function, num_proc=1)
    eval_dataset = eval_dataset.map(preprocess_function, num_proc=1)
