...
class ModelArguments:
    ...
    beta: float = field(default=1e-5, metadata={"help": "Beta for VAE's KL loss"})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    save_steps: int = field(default=2500)
...
def parse_args():
    ...
    parser.add_argument(
        "--model_name_or_path", type=str, required=True, help="Pretrained model path."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Can be 'cot_only', 'full_format', or 'q_q_only'.",
    )
    parser.add_argument(
        "--test_size", type=int, default=0.1, help="Test dataset split ratio."
    )
    parser.add_argument(
        "--max_steps", type=int, default=5000, help="Maximum training steps."
    )
    parser.add_argument(
        "--num_train_epochs", type=int, default=10, help="Number of training epochs."
    )
    parser.add_argument(
        "--learning_rate", type=float, default=2.5e-5, help="Learning rate."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="{}",
        help="JSON string of LR scheduler kwargs.",
    )
    parser.add_argument("--warmup_steps", type=int, default=0, help="Warmup steps.")
    parser.add_argument(
        "--optim", type=str, default="adamw_torch", help="Optimizer type."
    )
    parser.add_argument(
        "--weight_decay", type=float, default=0.0, help="Weight decay coefficient."
    )
    parser.add_argument(
        "--eval_strategy", type=str, default="epoch", help="Evaluation strategy."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--beta", type=float, default=1e-5, help="Beta value for KL loss"
    )
    # parser.add_argument(
    #     "--ddp_backend",
    #     type=str,
    ...
    #     "--fsdp",
    #     type=str,
    #     default="hybrid_shard auto_wrap",
    #     help="FSDP mode/configuration.",
    # )
    ...
    model_args = ModelArguments(
        **{k: v for k, v in vars(args).items() if k in ModelArguments.__annotations__}
    )
    training_args = TrainingArguments(
        **{
            k: v
            for k, v in vars(args).items()
            if k not in ModelArguments.__annotations__ and k not in skip_list
        }
    )
