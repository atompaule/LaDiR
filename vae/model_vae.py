...
class VAE(torch.nn.Module):
    ...
    def __init__(self, model_args, training_args, lora_config):
        ...
        self.mean = nn.Linear(
            in_features=self.icae.config.hidden_size,
        ...
        self.decompress_layer = nn.Linear(
            in_features=self.dim,
            out_features=self.icae.config.hidden_size,
            dtype=torch.bfloat16,
        )
        ...
        self.memory_token_embed = nn.Embedding(
            self.mem_size + 1, self.dim, padding_idx=None
        )
    ...
    def forward(
        ...
        for segment_idx in range(num_segments):
            ...
            segment_input_ids = torch.cat(
                [segment_input_ids, self.append_sequence.repeat(batch_size, 1)], dim=1
            )
            ...
            segment_input_embedding[mem_flag] = self.decompress_layer(
            ...
            batch_memory_tokens = segment_compress_outputs[mem_flag].view(
                batch_size, self.mem_size, -1
            )
        ...
        prompt_answer_embs[decoder_mem_flag] = decompress_outputs.view(
            -1, decompress_outputs.size(-1)
        )

        special_prompt = prompt_answer_ids >= self.vocab_size_with_mem

        prompt_answer_embs[special_prompt] = (
            self.decompress_layer(
                self.memory_token_embed(
                    prompt_answer_ids[special_prompt] - self.vocab_size
                )
            )
            .view(-1, decompress_outputs.size(-1))
            .to(prompt_answer_embs)
        ...
        effective_logits = logits[:, :-1, :].reshape(
            -1, logits.size(-1)
        )  # Why are we skipping the last generated logit? It's probably the eos token.
        # print("effective_logits shape: ", effective_logits.size())
        target_ids = labels[:, 1:].reshape(
            -1
        )  # Why does it take from the first index onwards?
    ...
    def decoder_loss(self, memory_slots, prompt_answer_ids, labels):
        ...
        prompt_answer_embs[special_prompt] = self.decompress_layer(
            self.memory_token_embed(
                prompt_answer_ids[special_prompt] - self.vocab_size
            ).to(prompt_answer_embs.dtype)
        ).view(
            -1, decompress_outputs.size(-1)
        )  # replace special token's embedding from self.memory_token_embed
        prompt_answer_embs = prompt_answer_embs.reshape(
            batch_size, -1, decompress_outputs.size(-1)
        )
    ...
    def tokens_to_embeddings(
        ...
        embeddings[special_flags] = self.decompress_layer(
            self.memory_token_embed(token_ids[special_flags] - self.vocab_size).to(
                embeddings
            )
        )  # replace special token's embedding from self.memory_token_embed
        return embeddings
    ...
    def _compress(
        self, input_ids: torch.LongTensor = None, return_sample=None
    ...
    def decode_text(self, memory_slots):
        with torch.no_grad():
            prompt_ids = (
                torch.LongTensor([[self.ae_token_id]])
                .to(self.device)
                .repeat(memory_slots.shape[0], 1)
            )

            prompt_answer_embs = self.tokens_to_embeddings(prompt_ids)

            memory_slots = memory_slots.to(
                device=self.device, dtype=prompt_answer_embs.dtype
            )
            ...
            generated_text = self.tokenizer.decode(
                generate_text, skip_special_tokens=True
            )
    ...
    def decode_text_batch(self, memory_slots):
        ...
        with torch.no_grad():
            ...
            for i in range(100):
                ...
                if active_sequences.any():
                    ...
                    next_token_embeddings = (
                        self.icae.get_base_model()
                        .model.embed_tokens(next_token_ids)
                        .unsqueeze(1)
                        .to(self.device)
                    )
            ...
            generated_texts = [
                self.tokenizer.decode(text, skip_special_tokens=True)
